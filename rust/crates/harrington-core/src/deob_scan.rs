...
pub(crate) fn normalize_liberal_url_token(token: &str) -> Option<String> {
    ...
    if let Some(separator) = ["&&", "||", "|"]
        .iter()
        .filter_map(|separator| token.find(separator))
        .min()
    {
...
fn scan_anti_recovery(deobfuscated: &str, env: &mut Environment) {
    ...
    static PATTERNS: Lazy<Vec<(Regex, &str)>> = Lazy::new(|| {
        vec![
            ...
            (
                Regex::new(
                    r"(?i)\b(?:Get-WmiObject|gwmi|Get-CimInstance|gcim)\b[^\r\n;&]*\bWin32_ShadowCopy\b[^\r\n;&]*(?:\.Delete\s*\(|Remove-CimInstance\b|Remove-WmiObject\b|rwmi\b)",
                )
                .unwrap(),
...
fn scan_account_modification(deobfuscated: &str, env: &mut Environment) {
    ...
    static PS_SET_LOCAL_USER_RE: Lazy<Regex> = Lazy::new(|| {
        Regex::new(r#"(?im)^[^\r\n]*?\bSet-LocalUser\b[^\r\n]*"#)
            .expect("powershell set-localuser regex")
    });
    ...
    for caps in PS_NEW_LOCAL_USER_RE.captures_iter(deobfuscated) {
        ...
        for command in powershell_statement_segments(m.as_str()) {
            let command = command.trim();
            if !contains_ascii_keyword(command, "New-LocalUser") {
                continue;
            }
            ...
            push("local-user-add", account, None, command.to_string());
        }
    }
    for caps in PS_ENABLE_LOCAL_USER_RE.captures_iter(deobfuscated) {
        ...
        for command in powershell_statement_segments(m.as_str()) {
            let command = command.trim();
            if !contains_ascii_keyword(command, "Enable-LocalUser") {
                continue;
            }
            ...
            for account in accounts {
                push("local-user-enable", account, None, command.to_string());
            }
        }
    }
    for caps in PS_SET_LOCAL_USER_RE.captures_iter(deobfuscated) {
        ...
        for command in powershell_statement_segments(m.as_str()) {
            let command = command.trim();
            if !contains_ascii_keyword(command, "Set-LocalUser") {
                continue;
            }
    ...
    for caps in PS_ADD_LOCALGROUP_MEMBER_RE.captures_iter(deobfuscated) {
        ...
        for command in powershell_statement_segments(m.as_str()) {
            let command = command.trim();
            if !contains_ascii_keyword(command, "Add-LocalGroupMember") {
                continue;
            }
            let mut positional =
...
mod account_modification_prefilter_tests {
    ...
    fn prefilter_allows_net_user_and_localgroup_adds() {
        for sample in [
            ...
            r#"Set-LocalUser -Name support -Password $p"#,
        ] {
...
fn scan_enumeration(deobfuscated: &str, env: &mut Environment) {
    ...
    static PATTERNS: Lazy<Vec<(Regex, &str, bool)>> = Lazy::new(|| {
        vec![
            ...
            (
                Regex::new(r"(?i)\b(?:Get-Command|gcm)\b[^\r\n]*").unwrap(),
                "ps-get-command",
                false,
            ),
            (
                Regex::new(r"(?i)\b(?:Get-Alias|gal)\b[^\r\n]*").unwrap(),
                "ps-get-alias",
                false,
            ),
            (
...
fn has_enumeration_atom(text: &str) -> bool {
    ...
    #[allow(clippy::expect_used)]
    static ENUMERATION_ATOM_RE: Lazy<RegexSet> = Lazy::new(|| {
        let atoms = [
            "net user",
            "net.exe user",
            "net group",
        ...
        let patterns = atoms
            .iter()
            .map(|atom| format!("(?i){}", regex::escape(atom)))
            .collect::<Vec<_>>();
        RegexSet::new(patterns).expect("enumeration atom regex set")
    });

    ENUMERATION_ATOM_RE.is_match(text)
}
