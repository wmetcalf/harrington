...
fn parse_dotnet_layout(bytes: &[u8]) -> Result<DotnetLayout, DotnetError> {
    ...
    let pe_end = pe_off.checked_add(4).ok_or(DotnetError::Bounds)?;
    if bytes.get(pe_off..pe_end) != Some(b"PE\0\0") {
        return Err(DotnetError::NotPe);
    }
    let coff_off = pe_end;
    let num_sections = rd_u16(bytes, coff_off.checked_add(2).ok_or(DotnetError::Bounds)?)? as usize;
    let opt_hdr_size =
        rd_u16(bytes, coff_off.checked_add(16).ok_or(DotnetError::Bounds)?)? as usize;
    let opt_off = coff_off.checked_add(20).ok_or(DotnetError::Bounds)?;
    let magic = rd_u16(bytes, opt_off)?;
    let data_dir_off = match magic {
        0x10b => opt_off.checked_add(96).ok_or(DotnetError::Bounds)?,
        0x20b => opt_off.checked_add(112).ok_or(DotnetError::Bounds)?,
        _ => return Err(DotnetError::NotPe),
    };
    let cli_dir = data_dir_off
        .checked_add(14usize.checked_mul(8).ok_or(DotnetError::Bounds)?)
        .ok_or(DotnetError::Bounds)?;
    let cli_rva = rd_u32(bytes, cli_dir)?;
    let cli_size = rd_u32(bytes, cli_dir.checked_add(4).ok_or(DotnetError::Bounds)?)?;
    ...
    let metadata_rva = rd_u32(bytes, cli_off.checked_add(8).ok_or(DotnetError::Bounds)?)?;
    let metadata_size =
        rd_u32(bytes, cli_off.checked_add(12).ok_or(DotnetError::Bounds)?)? as usize;
    let metadata_off = rva_to_offset(metadata_rva, &sections).ok_or(DotnetError::Bounds)?;
    let metadata_end = metadata_off
        .checked_add(metadata_size)
        .ok_or(DotnetError::Bounds)?;
    if metadata_end > bytes.len() || bytes.get(metadata_off..metadata_off + 4) != Some(b"BSJB") {
        return Err(DotnetError::NotClr);
    }
...
pub fn extract_us_strings(bytes: &[u8]) -> Result<Vec<String>, DotnetError> {
    ...
    let magic = rd_u16(bytes, opt_off)?;
    let data_dir_off = match magic {
        0x10b => opt_off.checked_add(96).ok_or(DotnetError::Bounds)?, // PE32
        0x20b => opt_off.checked_add(112).ok_or(DotnetError::Bounds)?, // PE32+
        _ => return Err(DotnetError::NotPe),
    };
    ...
    let metadata_size =
        rd_u32(bytes, cli_off.checked_add(12).ok_or(DotnetError::Bounds)?)? as usize;
    let metadata_off = rva_to_offset(metadata_rva, &sections).ok_or(DotnetError::Bounds)?;
    let bsjb_end = metadata_off.checked_add(4).ok_or(DotnetError::Bounds)?;
    if bytes.get(metadata_off..bsjb_end) != Some(b"BSJB") {
        return Err(DotnetError::NotClr);
    }
    let metadata_end = metadata_off
        .checked_add(metadata_size)
