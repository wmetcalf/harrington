...
pub fn gunzip(input: &[u8], max_out: usize) -> Result<Vec<u8>, CryptoError> {
    ...
    // Pre-reserve the full cap so Vec doubling never overshoots `max_out`.
    // Without this, the last chunk could trigger `Vec::extend_from_slice` to
    // realloc capacity to roughly 2× max_out before the next iteration's
    // size check rejects — needlessly committing memory beyond the budget.
