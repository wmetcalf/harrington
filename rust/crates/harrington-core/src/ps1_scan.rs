...
static FILE_B64_XOR_LOADER_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r#"(?is)\$([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(\d{1,3})\s*;.*?\$([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(?:\(\s*(?:gc|Get-Content)\s*['"]([^'"]+)['"]\s*\)\s*-join\s*['"]{2}|(?:gc|Get-Content)\s+-Raw\s+['"]([^'"]+)['"]|(?:gc|Get-Content)\s+['"]([^'"]+)['"]\s+-Raw\b).*?\[Convert\]::FromBase64String\s*\(\s*\$([A-Za-z_][A-Za-z0-9_]*)\s*\).*?-bxor\s*\$([A-Za-z_][A-Za-z0-9_]*)"#,
    )
