...
fn caret_postprocess(s: &str) -> String {
    ...
    while i < bytes.len() {
        ...
        if byte == b'^' && !in_dq {
            if let Some((next, next_len)) = next_char_at(s, i + 1) {
                out.push(next);
                i += 1 + next_len;
            } else {
            ...
            let Some((c, len)) = next_char_at(s, i) else {
                break;
            };
            out.push(c);
            i += len;
...
fn is_base64_fragment_set_assignment(text: &str) -> bool {
    ...
    let Some(&a) = bytes.first() else {
        return false;
    };
    let Some(&b) = bytes.get(1) else {
        return false;
    };
    let Some(&c) = bytes.get(2) else {
        return false;
    };
    if !(a.eq_ignore_ascii_case(&b's')
        && b.eq_ignore_ascii_case(&b'e')
        && c.eq_ignore_ascii_case(&b't'))
    {
        return false;
    }
    let rest = &trimmed[3..];
...
fn value_likely_has_nested_ref(s: &str) -> bool {
    ...
    let bytes = s.as_bytes();
    let mut i = 0;
    while let Some(&sigil) = bytes.get(i) {
        if sigil == b'%' || sigil == b'!' {
            ...
            if has_name && bytes.get(j) == Some(&sigil) {
                return true;
            }
        }
        i += 1;
        ...
        match c {
            '%' => {
                ...
                if bytes.get(i + 1) == Some(&b'%') {
                    ...
                    if bytes.get(k) == Some(&b'~') {
                        ...
                        while let Some(&b) = bytes.get(k) {
                            if matches!(
                                b,
                                b'f' | b'd' | b'p' | b'n' | b'x' | b's' | b'a' | b't' | b'z' | b'$'
                            ) {
                                k += 1;
                            } else {
                                break;
                    ...
                    if let Some(c2) = bytes.get(k).map(|b| *b as char) {
                        if c2.is_ascii_alphabetic() {
                            // %%X:op%% — substitute/substring on FOR var.
                            if !had_tilde && bytes.get(k + 1) == Some(&b':') {
                                let mut m = k + 2;
                                while m + 1 < bytes.len() {
                                ...
                                if m + 1 < bytes.len() && bytes[m] == b'%' && bytes[m + 1] == b'%' {
                                    let raw: String = s[i..m + 2].to_string();
                                    out.push_str(&raw);
                                    i = m + 2;
                                    continue;
                                }
                            ...
                            let raw: String = s[i..=k].to_string();
                            out.push_str(&raw);
                            i = k + 1;
                            continue;
                        }
                ...
                if bytes.get(i + 1) == Some(&b'~') {
                    ...
                    if let Some((rendered, next)) = render_percent_tilde_from_chars(&chars, 0, env)
                    {
...
pub(crate) fn apply_substr(s: &str, index: i64, length: Option<i64>) -> String {
    ...
    let start_byte = char_to_byte_offset(s, start as usize);
    let end_byte = char_to_byte_offset(s, end as usize);
...
pub(crate) fn apply_substitute(s: &str, needle: &str, repl: &str, wildcard: bool) -> String {
    ...
    if wildcard {
        if needle.is_ascii() {
            if let Some(pos) = crate::util::find_ascii_case_insensitive_from(s, needle, 0) {
    ...
    if needle.is_ascii() {
        ...
        let find = crate::util::find_ascii_case_insensitive_from;
        while let Some(pos) = find(s, needle, search_from) {
            out.push_str(&s[search_from..pos]);
            out.push_str(repl);
...
mod dosfuscation_tests {
    ...
    use crate::normalize::{apply_substitute, apply_substr};
    ...
    #[test]
    fn nested_var_ref_in_unicode_value_still_re_expands() {
        let mut env = Environment::new(&Config::default());
        env.set("a", "foo");
        env.set("b", "hé%a%llo");
        let got = normalize_to_string(&lex("%b%"), &mut env);
        assert_eq!(got, "héfoollo", "got: {:?}", got);
    }

    #[test]
    fn literal_digit_after_substring_ref_survives_before_empty_var() {
        // xeno.bat builds a `.ps1` suffix as `%ProgramFiles(x86):~-7,1%1%noise%`.
        // The `1` is literal text between two percent refs, not `%1`.
        let got = nm("%ProgramFiles(x86):~-7,1%1%noise%");
        assert_eq!(got, "s1", "got: {:?}", got);
    }
    ...
    #[test]
    fn ascii_substitute_replaces_all_matches_case_insensitively() {
        assert_eq!(apply_substitute("cmd/CMD/cMd", "cmd", "x", false), "x/x/x");
        assert_eq!(apply_substitute("hécmdllo", "cmd", "x", false), "héxllo");
    }
    ...
    #[test]
    fn substr_ascii_fast_path_and_unicode_fallback() {
        assert_eq!(apply_substr("ABCDE", 1, Some(3)), "BCD");
        assert_eq!(apply_substr("héllo", 1, Some(3)), "éll");
        assert_eq!(apply_substr("é🙂abc", 1, Some(2)), "🙂a");
    }
    ...
    #[test]
    fn caret_escape_with_unicode_neighbor_is_preserved() {
        let out = normalize_to_string(
            &lex("echo hé^>llo"),
            &mut Environment::new(&Config::default()),
        );
        assert_eq!(out, "echo hé>llo");
    }
