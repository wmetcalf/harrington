...
fn scan_process_url_arguments(deobfuscated: &str, env: &mut Environment) {
    ...
    for line in deobfuscated.lines() {
        ...
        for i in 0..tokens.len() {
            let cmd = command_name(strip_outer_quotes(&tokens[i]));
            if !cmd.eq_ignore_ascii_case("certreq") && !cmd.eq_ignore_ascii_case("certreq.exe") {
                continue;
            }
    ...
    while i < tokens.len() {
        ...
        if !skipped_title
            && tokens
                .get(i + 1)
                .map(|next| {
                    let next = strip_outer_quotes(next);
...
fn scan_certoc_deob_text(deobfuscated: &str, env: &mut Environment) {
    ...
    for line in deobfuscated.lines() {
        ...
        for i in 0..tokens.len() {
            let cmd = command_name(strip_outer_quotes(&tokens[i]));
            if !cmd.eq_ignore_ascii_case("certoc") && !cmd.eq_ignore_ascii_case("certoc.exe") {
                continue;
            }
...
fn scan_desktopimgdownldr_deob_text(deobfuscated: &str, env: &mut Environment) {
    ...
    for line in deobfuscated.lines() {
        ...
        for i in 0..tokens.len() {
            let cmd = command_name(strip_outer_quotes(&tokens[i]));
            if !cmd.eq_ignore_ascii_case("desktopimgdownldr")
                && !cmd.eq_ignore_ascii_case("desktopimgdownldr.exe")
            {
                continue;
            }
...
fn scan_echoed_vbs_deob_text(deobfuscated: &str, env: &mut Environment) {
    ...
    let has_vbs_downloader = has_echoed_vbs_xmlhttp_shape(deobfuscated);
...
fn scan_copied_extrac32_alias_deob_text(deobfuscated: &str, env: &mut Environment) {
    ...
    for t in &env.traits {
        ...
        if !src_base.eq_ignore_ascii_case("extrac32")
            && !src_base.eq_ignore_ascii_case("extrac32.exe")
        {
            continue;
        }
        let Some(dst_base) = basename_trimmed(dst) else {
...
fn scan_copied_mshta_alias_deob_text(deobfuscated: &str, env: &mut Environment) {
    ...
    for t in &env.traits {
        ...
        if !src_base.eq_ignore_ascii_case("mshta") && !src_base.eq_ignore_ascii_case("mshta.exe") {
            continue;
        }
        let Some(dst_base) = basename_trimmed(dst) else {
            continue;
...
fn scan_copied_msiexec_alias_deob_text(deobfuscated: &str, env: &mut Environment) {
    ...
    for t in &env.traits {
        ...
        if !src_base.eq_ignore_ascii_case("msiexec")
            && !src_base.eq_ignore_ascii_case("msiexec.exe")
        {
            continue;
        }
        let Some(dst_base) = basename_trimmed(dst) else {
...
fn scan_copied_rundll32_alias_deob_text(deobfuscated: &str, env: &mut Environment) {
    ...
    for t in &env.traits {
        ...
        if !src_base.eq_ignore_ascii_case("rundll32")
            && !src_base.eq_ignore_ascii_case("rundll32.exe")
        {
            continue;
        }
        let Some(dst_base) = basename_trimmed(dst) else {
...
fn scan_inmem_assembly_load(deobfuscated: &str, env: &mut Environment) {
    ...
    for m in CUSTOM_LOADASSEMBLY_RE.find_iter(deobfuscated) {
        if match_line_starts_with_echo(deobfuscated, m.start()) {
            continue;
        }
        let variant = "LoadAssembly".to_string();
        if !seen.insert(variant.clone()) {
            continue;
        }
        if env.traits.iter().any(|t| {
...
fn scan_remote_exec(deobfuscated: &str, env: &mut Environment) {
    ...
    for caps in WINRM_RE.captures_iter(deobfuscated) {
            ...
            .unwrap_or(false)
        {
            ...
            .get(0)
            .map(|m| contains_ascii_case_insensitive(m.as_str(), "winrs"))
            .unwrap_or(false)
        {
            "winrs"
        } else if caps
...
fn scan_script_host_deob_text(deobfuscated: &str, env: &mut Environment) {
    if !contains_ascii_case_insensitive(deobfuscated, "cscript")
        && !contains_ascii_case_insensitive(deobfuscated, "wscript")
    {
        return;
    }
    ...
    for caps in SCRIPT_HOST_RE.captures_iter(deobfuscated) {
        ...
        if match_line_starts_with_echo(deobfuscated, cmd_match.start()) {
            continue;
        }
...
fn push_script_host_exec_trait(host: &str, src: String, env: &mut Environment) {
    let already = env.traits.iter().any(|t| match (host, t) {
        ("cscript", Trait::CscriptExec { src: s }) => s == &src,
        ("wscript", Trait::WscriptExec { src: s }) => s == &src,
        _ => false,
    });
    if already {
        return;
    ...
    match host {
        "cscript" => env.traits.push(Trait::CscriptExec { src }),
        "wscript" => env.traits.push(Trait::WscriptExec { src }),
        _ => {}
    }
}
